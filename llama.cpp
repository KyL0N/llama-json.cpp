...
static bool llama_model_load(const std::string& fname, llama_context& lctx, int n_ctx, int n_parts, ggml_type memory_type, bool vocab_only)
{
                ...
                if (n_dims == 1) {
                    if (tensor->ne[0] != ne[0] || tensor->ne[1] != ne[1]) {
                        fprintf(stderr, "%s: tensor '%s' has wrong shape in model file: got [%d, %d], expected [%d, %d]\n", __func__, name.data(),
                                tensor->ne[0], tensor->ne[1], ne[0], ne[1]);
                        return false;
                    }
                }
                else {
                    if (split_type == 0) {
                        if (tensor->ne[0] / n_parts != ne[0] || tensor->ne[1] != ne[1]) {
                            fprintf(stderr, "%s: tensor '%s' has wrong shape in model file: got [%d, %d], expected [%d, %d]\n", __func__, name.data(),
                                    tensor->ne[0] / n_parts, tensor->ne[1], ne[0], ne[1]);
                            return false;
                        }
                    }
                    else {
                        if (tensor->ne[0] != ne[0] || tensor->ne[1] / n_parts != ne[1]) {
                            fprintf(stderr, "%s: tensor '%s' has wrong shape in model file: got [%d, %d], expected [%d, %d]\n", __func__, name.data(),
                                    tensor->ne[0], tensor->ne[1] / n_parts, ne[0], ne[1]);
                            return false;
                        }
                    }
                }
...
//
static bool llama_eval_internal(llama_context& lctx, const llama_token* tokens, const int n_tokens, const int n_past, const int n_threads)
{
    const int64_t t_start_us = ggml_time_us();
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // self-attention
        {
            ...
            struct ggml_tensor* Q = ggml_permute(
                ctx0, ggml_rope(ctx0, ggml_cpy(ctx0, Qcur, ggml_new_tensor_3d(ctx0, GGML_TYPE_F32, n_embd / n_head, n_head, N)), n_past, n_rot, 0), 0, 2, 1, 3);
...
static void sample_top_k(std::vector<std::pair<double, llama_vocab::id>>& logits_id, int top_k)
{
    ...
    std::partial_sort(logits_id.begin(), logits_id.begin() + top_k, logits_id.end(),
                      [](const std::pair<double, llama_vocab::id>& a, const std::pair<double, llama_vocab::id>& b) { return a.first > b.first; });

    logits_id.resize(top_k);
}

static llama_vocab::id
llama_sample_top_p_top_k(llama_context& lctx, const std::vector<llama_vocab::id>& last_n_tokens, int top_k, double top_p, double temp, double repeat_penalty)
{
    auto& rng = lctx.rng;
...
llama_token llama_sample_top_p_top_k(llama_context*     ctx,
                                     ...
                                     double             repeat_penalty)
{
    ...
    result = llama_sample_top_p_top_k(*ctx, last_n_tokens, top_k, top_p, temp, repeat_penalty);
...
void llama_print_timings(struct llama_context* ctx)
{
    ...
    fprintf(stderr, "%s:   sample time = %8.2f ms / %5d runs (%8.2f ms per run)\n", __func__, 1e-3f * ctx->t_sample_us, n_sample,
            1e-3f * ctx->t_sample_us / n_sample);
